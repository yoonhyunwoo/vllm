...
def test_advance_grammar_marker_last_trims_to_empty():
    """Regression for #44006: when the reasoning-end marker is the last
    token in the step (the [198, </think>] crash shape), trim returns
    empty and advance_grammar skips accept_tokens entirely.
    """
    tokenizer, manager, request, prompt, marker = _setup_boundary_request(
        "xgrammar"
    )
    structured_req = request.structured_output_request
    pre = tokenizer.encode(" ")[0]

    step_tokens = [pre, marker]
    request.append_output_token_ids(step_tokens)

    # Marker is the last token → trim produces empty → grammar untouched.
    assert manager.advance_grammar(request, step_tokens) is True
    assert structured_req.reasoning_ended is True
    assert structured_req.reasoning_end_token_index == len(prompt) + 1
    assert not structured_req.grammar.is_terminated()
